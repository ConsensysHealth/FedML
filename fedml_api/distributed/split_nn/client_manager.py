...
class SplitNNClientManager(ClientManager):
    ...
    def run_forward_pass(self):
        ...
        # ToDo Change self.send_activations_to_facilitator(acts, self.trainer.FACILITATOR_RANK)
        self.send_activations_to_facilitator(acts, labels, self.trainer.FACILITATOR_RANK)
        self.trainer.batch_idx += 1


    # ToDo change send_activations_to_facilitator(self, acts: torch.Tensor, receive_id: int):
    def send_activations_to_facilitator(self, acts: torch.Tensor, labels: torch.Tensor, receive_id: int):
        ...
        # ToDo Change the (acts, labels) to only acts
        message.add_params(MyMessage.MSG_ARG_KEY_ACTS, (acts, labels)) #  message.add_params(MyMessage.MSG_ARG_KEY_ACTS, acts)
