...
class FedAVGServerManager(ServerManager):
    ...
    def handle_message_receive_model_from_client(self, msg_params):
        print('Server: recieved weights from a client.')
        ...
        if not b_all_received:
            print('Server: waiting for other clients to send weights.')
        logging.info("b_all_received = " + str(b_all_received))
        if b_all_received:
            print('Server: all weights recieved.')
            global_model_params = self.aggregator.aggregate()
            print('Server: testing on server for all clients.')
            self.aggregator.test_on_server_for_all_clients(self.round_idx)
            print('Server: done, starting next round.')
            ...
            if self.round_idx == self.round_num:
                ...
                self.finish()
                print('Server: my job is finished.')
                return
