...
def fed_learning_fixture_fit(federated_learning: VerticalMultiplePartyLogisticRegressionFederatedLearning, train_data: dict, test_data: dict, epochs=50, batch_size=-1):
    """
    Performs the fitting of the federated learning environment
    """
    logger.debug("Step 7: initialize FederatedLearningFixture")
    logger.debug("Step 9: start to fit, split data and refer back to VerticalMultiplePartyLogisticRegressionFederatedLearning")
    main_party_id = federated_learning.get_main_party_id()
    ...
    for ep in range(epochs):
        for batch_idx in range(n_batches):
            ...
            loss = federated_learning.fit(Y_batch,party_X_train_batch_dict, global_step)
            ...
            if (global_step + 1) % recording_period == 0:
                ...
                y_prob_preds = federated_learning.predict(party_X_test_dict)
