...
class FacilitatorManager(Observer):
    def __init__(self, args, comm=None, rank=0, size=0, backend="MPI"):
        ...
        elif backend == "MQTT": # This will be used in the future
            HOST = "0.0.0.0"
            # HOST = "broker.emqx.io"
            PORT = 1883
            self.com_manager = MqttCommManager(HOST, PORT, client_id=rank, client_num=size - 1)
        elif backend == "GRPC": # This will be used in the future
            HOST = "0.0.0.0"
            PORT = 50000 + rank
    ...
    def finish(self):
        logging.info("__finish facilitator")
        if self.backend == "MPI":
